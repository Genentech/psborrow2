test_that("set_covariates is working correctly", {

   # Make covariate objects
   covs1 <- set_covariates(covariates = c('a','b','c'),
                          priors = list(
   ...
   covs2 <- set_covariates(covariates = c('a','b','c'),
   ...
   # Errors
   expect_error(set_covariates(covariates = c('a','b'),
                               priors = list('a' = normal_prior(0, 20))),
                "Either specify 1 prior distribution applying to all covariates, or specify a named list with 1 prior per covariate"
   )

   expect_error(set_covariates(covariates = c('a','c'),
                               priors = list('a' = normal_prior(0, 20),
                                             'b' = normal_prior(0, 10))),
   ...
   expect_error(set_covariates(covariates = c('a','b'), 20),
