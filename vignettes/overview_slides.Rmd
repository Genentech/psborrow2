...
trt_from_anls <- function(borrowing, adjustment = NULL) {
  ...
  res <- mcmc_sample(anls, iter_warmup = 500, iter_sampling = 1000, chains = 1L)
  draws <- res$draws()
...
library(purrr)
res <- map2(
  list(no_borrow, no_borrow,
       bdb_borrow, bdb_borrow),
  list(NULL, adj_ps, NULL, adj_ps),
  trt_from_anls
)
