...
```{r include = FALSE}
if (!"gt" %in% installed.packages()[, "Package"]) {
  install.packages("gt", repos = c("CRAN" = "https://cran.r-project.org/"))
}
if (!"gtsummary" %in% installed.packages()[, "Package"]) {
  install.packages("gtsummary", repos = c("CRAN" = "https://cran.r-project.org/"))
}
```
...
uninf_gamma <- vapply(seq(1, 10000, 1), dgamma, shape = .001, rate = .001, FUN.VALUE = numeric(1))
...
Let's start by conducting a Cox proportional hazards model without using
the external data:

```{r message = FALSE, echo = FALSE, fig.align = "center", fig.dim = c(5, 2.7)}
library(survival)
library(survminer)
...
BDB. Maybe our outcome
distributions are dissimilar between internal and external controls:

```{r, fig.align="center", fig.dim = c(6,3.5), echo = FALSE}
ggsurvplot(
...
The outcomes are quite different (and we should have checked
this earlier)!

## Evaluating balance between cohorts

Our cohorts appear to differ in baseline characteristics:
...
```{r include = FALSE}
if (!"table1" %in% installed.packages()[, "Package"]) {
  install.packages("table1", repos = c("CRAN" = "https://cran.r-project.org/"))
}
```
...
Let's adjust for these imbalances with propensity score adjustment:
...
trt_from_anls <- function(borrowing, adjustment = NULL) {
  ...
  tibble(
    adjustment = ifelse(is.null(adjustment), "No adjustment", "PS adjustment"),
...
res_df$adjustment <- factor(res_df$adjustment,
  levels = c("No adjustment", "PS adjustment")
)
...
forest_plot <- ggplot(res_df) +
  geom_vline(aes(xintercept = hr, color = Reference), linetype = 2, data = data.frame(
    Reference = c("Null effect", "True HR"),
    hr = c(1.0, 0.7)
  )) +
