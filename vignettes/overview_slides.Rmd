...
randomized, controlled trial (RCT) to increase study power and
shorten trial duration or reduce the number of subjects needed to enroll.
...
Consider the scenario just described. How best should we incorporate the
knowledge we have on the external control arm?

`psborrow2` allows the user to evaluate and compare three different
...
external data. We will assume two parameters of interest:
...
As $\tau$ approaches infinity, the hierarchy collapses and
the prior distribution on $\rho_{00}$ effectively
becomes:
...
```{r fig.align = "center", fig.dim = c(6,4)}
plot(gamma_prior(alpha = 0.001, beta = 0.001))
```
