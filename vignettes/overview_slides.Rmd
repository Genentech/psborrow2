...
- $\rho_{00}$, the log hazard rate for the internal control
- $\rho_{10}$, the log hazard rate for the internal experimental

The hazard ratio, $\theta$, is therefore:
$$
\theta = e^{\frac{\rho_{10}}{\rho_{00}}}
$$
...
- $\rho_{01}$, the log hazard rate of the external control cohort
...
There appear to be some differences. Let's use inverse probability of treatment 
weighting (IPTW) using the ATT estimand to adjust for confounding. We first 
...
We'll make a new matrix called `example_matrix_ps` includes the 
IPTW weights:
...
```{r}
tail(example_matrix_ps)
```

## Weighted KM curves

The weighted curves look much more similar!

```{r message = FALSE, echo = FALSE, fig.align = "center", fig.dim = c(7, 4)}
km_fit_ps <- survfit(Surv(time = time, event = 1 - cnsr) ~ trt + ext, 
                     data = example_dataframe, 
                     weights = example_dataframe$att)
ggsurvplot(fit = km_fit_ps, data = example_dataframe, palette = "Set1")
```


## Weighted analysis object

Now let's create an `Analysis` object that adjusts for propensity scores by 
passing the weights to `exp_surv_dist`:
...
It looks like we see more of a treatment effect when differences in the
...
trt_from_anls <- function(borrowing, exp_outcome) {
  require(posterior)
  require(tibble)
  ...
  tibble(
    adjustment = ifelse(exp_outcome@weight_var == "", "No weighting", "IPTW-ATT weights"),
...
exp_outcome_weight <- exp_surv_dist(
  time_var = "time",
  cens_var = "cnsr",
  baseline_prior = normal_prior(0, 10000),
  weight_var = "att"
...
res <- map2(
  ...
  list(exp_outcome, exp_outcome_weight, exp_outcome, exp_outcome_weight, exp_outcome, exp_outcome_weight, exp_outcome, exp_outcome_weight),
  trt_from_anls
)

res_df <- bind_rows(res)
res_df$adjustment <- factor(res_df$adjustment,
  levels = c("No weighting", "IPTW-ATT weights")
)
