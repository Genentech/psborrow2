...
the histogram of draws for our estimand of interest, the treament hazard
ratio:
...
ggsurvplot(survfit(Surv(time, 1-cnsr) ~ ext, example_dataframe,
                   subset = trt == 0),
           example_dataframe)
```
...
anls_ps_no_borrow <- create_analysis_obj(
  ...
  covariates = add_covariates(c("ps_cat(0.2,0.4]", 
                                "ps_cat(0.4,0.8]", 
                                "ps_cat(0.8,1]"),
...
```{r}
single_mat <- sim_single_matrix(true_hr = 0.6,
                                drift_hr = 1.0)
```
...
simulation_obj <- create_simulation_obj(
  ...
  treatment = treatment_details(trt_flag_col = "trt", 
                                trt_prior = normal_prior(0, 10000))
)
```
