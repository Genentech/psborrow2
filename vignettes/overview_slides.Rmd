...
The flags in this `data.frame` are explained in `?example_matrix`. 

For now, we need to understand that `trt` is a flag denoting receipt of the
...
It looks like we have 48 patients in our internal control,
95 in our internal experimental, and 357 in our external control.
...
```{r message = FALSE, echo = FALSE, fig.align = "center", fig.dim = c(5, 3)}
library(survival)
library(survminer)
...
fully parametric Bayesian model, let's also evaluate the internal results 
...
exp_fit <- flexsurvreg(Surv(time = time, event = 1 - cnsr) ~ trt,
  dist = "exponential",
  data = filter(example_dataframe, ext == 0),
)

tidy(exp_fit, conf.int = TRUE, transform = "coefs.exp") %>% 
  gt() %>% 
  fmt_number(columns = 2:7, decimals = 2)
```
...
Here, the hazard ratio is very similar to the Cox proportional hazards 
model: 0.88 (95% CI 0.62 - 1.25).

## Hybrid control analysis

For a BDB analyiss, we want to create an object of class `Analysis`
with `create_analysis_obj()`. There are five 
required arguments: 
...
`Prior` class objects. These all follow a naming convention of: 
distribution name + `_prior()`, e.g.:
...
Each `Prior` object has a `plot` method. Let's confirm that a normal distribution
centered at 0 and with wide variance appears to be uninformative:
...
For the sake of example, let's also see what an informative normal prior may
look like:
...
`borrowing_details`. We need to specify  three arguments:
...
We'll then incorporate the propensity scores back into our example `data.frame`
and create propensity score quartiles for which we'll adjust.
```{r include = FALSE}
...
For this data, we need to create a `guide` that explains
