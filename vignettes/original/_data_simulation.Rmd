...
We can control how datasets are generated with the `n`, `treatment_effect` and `drift_hr` parameters of `generate()`. These
will override the defaults specified in `create_data_simulation()`.
```{r}
generate(data_sim, n = 10, treatment_effect = c(1, 1.3, 2), drift_hr = c(1, 1.2))
```
...
my_data <- generate(my_data_sim_setup, n = 3, treatment_hr = 1, drift_hr = c(0, 1))
```
...
my_data_sim_setup
my_data <- generate(my_data_sim_setup, n = 3, treatment_hr = 1.3, drift_hr = c(1, 1.2))
```
