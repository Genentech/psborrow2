...
```{r psborrow2, echo = FALSE, message = FALSE}
library(psborrow2)
```
...
library(survival)
library(ggsurvfit)
km_fit <- survfit2(Surv(time = time, event = 1 - cnsr) ~ trt + ext, example_dataframe)
ggsurvfit(km_fit)
```
<br>
...
gt(
  tribble(
    ...
    "outcome", paste0(
      "Object of class Outcome as output by outcome_surv_exponential(), ",
      "outcome_surv_weibull_ph(), or outcome_bin_logistic()."
    ),
...
trt_from_anls <- function(borrowing, adjustment = NULL) {
  ...
  res <- mcmc_sample(anls, iter_warmup = 1000, iter_sampling = 1000, chains = 4L, seed = 123)
  draws <- res$draws()
