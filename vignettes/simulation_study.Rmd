...
`data_list` is where you input the data you will be using for the
...
`sim_outcome_list()` is where you input outcome details, for instance
the type of outcome and prior distributions on the baseline outcome
rate as well as any ancillary parameters. In `psborrow2` currently, there
are three outcome types supported:

* Exponential survival distribution (`exp_surv_dist()`)
* Weibull proportional hazards survival distribution (`weib_ph_surv_dist()`)
* Bernoulli distribution with a logistic link (logistic regression,
`logistic_bin_outcome()`)

For our example, let's compare Weibull and exponential survival
distributions.

```{r}
sol <- sim_outcome_list(
  list(
...
`sim_borrowing_list()` is where you input borrowing details, including
the type of borrowing from among:

* No borrowing
* Full borrowing
* BDB

In addition, the prior on the commensurability parameter may also be
varied. For our example, let's compare four scenarios:
...
`sim_treatment_list()` is where you input the treatment details for a simulation study. Of most
importance in this field is specifying the prior distribution for the treatment effect. Let's
keep an uninformative prior in this example:

```{r}
stl <- sim_treatment_list(
  list("Standard" = treatment_details("trt", normal_prior(0, 1000)))
)
stl
```

## `create_simuilation_obj()`
