...
of true HRs of 0.6 and 0.8 and drift HRs of 0.5, 1.0, and 1.5. That gives us
6 total scenarios, and we'll simulate 10 datasets within each scenario.
...
data_list <- list(
  ...
  lapply(1:10, function(z) sim_single_matrix(true_hr = 0.8)),
  lapply(1:10, function(z) sim_single_matrix(true_hr = 0.6, drift_hr = 1.5)),
  lapply(1:10, function(z) sim_single_matrix(true_hr = 0.8, drift_hr = 1.5)),
  lapply(1:10, function(z) sim_single_matrix(true_hr = 0.6, drift_hr = 0.5)),
  lapply(1:10, function(z) sim_single_matrix(true_hr = 0.8, drift_hr = 0.5))
)
```
...
two values: 0.6 and 0.8. We varied the drift HR so it took three values: 
0.5, 1.0, and 1.5. Our guide should therefore be six rows to represent
the six scenarios. We pass `c(1L, 2L, 3L, 4L, 5L, 6L)` onto `index` to clarify that
`data_list[[1L]]` corresponds to the settings in the first row,
`data_list[[2L]]` to the second row, and so on.

```{r}
guide <- data.frame(
  trueHR = c(0.6, 0.8, 0.6, 0.8, 0.6, 0.8),
  driftHR = c(1.0, 1.0, 1.5, 1.5, 0.5, 0.5),
  index = c(1L, 2L, 3L, 4L, 5L, 6L)
)
...
when the true hazard ratio was greater.
