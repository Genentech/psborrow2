...
We can also use weights to specify a fixed power prior model. Here we set $alpha = 0.1$ for external controls.

```{r}
example_matrix_pp01 <- cbind(example_matrix, pp = ifelse(example_matrix[, "ext"] == 1, 0.1, 1))

analysis_pp01 <- create_analysis_obj(
  data_matrix = example_matrix_pp01,
  outcome = exp_surv_dist("time", "cnsr", normal_prior(0, 10000), weight_var = "pp"),
  borrowing = borrowing_details("Full borrowing", "ext"),
  treatment = treatment_details("trt", normal_prior(0, 10000))
)
...
plot(density(models[[1]]$draws("HR_trt")),
  col = 1, xlim = c(0, 2), ylim = c(0, 9),
  xlab = "HR_trt",
  main = "Posterior Hazard Ratio"
)
for (i in 2:8) {
  lines(density(models[[i]]$draws("HR_trt")), col = i, lty = i)
}
legend("topright", col = seq_along(models), lty = seq_along(models), legend = model_names)
```
