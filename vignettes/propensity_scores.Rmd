...
Propensity scores (PS) methods offer various ways to adjust analyses for differences in groups of patients. For external control analyses, we are interested in the differences between the internal patients and the external patients.

@wang2021 discuss using propensity scores for IPTW, matching and stratification in combination with a Bayesian analysis. These methods allow for the separation of the design and analysis into two stages, which may be attractive in a regulatory setting.
...
The simplest approach for calculating PS is a logistic generalized linear model.
First, let's load some demo data from `psborrow2`.
...
Let's now fit a logistic generalized linear model and add the PS
...
The calculated PS can be included as a covariate in the analysis object
...
The [PSweight](https://cran.r-project.org/package=PSweight) package can calculate PS using other model fitting approaches than the simple glm described above. It also provides some summary statistics and plots.
