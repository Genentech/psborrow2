---
title: "Conduct a hybrid control analysis on a dataset using BDB"
...
analyses on data using `psborrow2`.
...
For an analysis on data in `psborrow2`, we need to create an object
...
***Note*** Only `numeric` matrices can be are supported. See the [Example data] for creating such a matrix
`from a data.frame`
...
odds instead). Let's assume we have no prior knowledge on this event rate, so we'll
...
Note that depending on your computer running this will take a few minutes.
...
objects to be more interpretable, `rename_draws_covariates()`, which uses
the `variable_dictionary` labels.
...
Alternatively, you can use [posterior::rename_variables()] to specify your own names.
