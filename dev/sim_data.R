...
res_noborrow <- mcmc_sample(anls_noborrow, iter_sampling = 10000, chains = 4)
...
res_borrow <- mcmc_sample(anls_borrow, iter_sampling = 10000, chains = 4)
...
res_borrow_cov <- mcmc_sample(anls_borrow_cov, iter_sampling = 10000, chains = 4)

res_noborrow_tbl <- summarize_draws(res_noborrow$draws(), ~ quantile(.x, probs = c(0.025, 0.975)))
res_borrow_tbl <- summarize_draws(res_borrow$draws(), ~ quantile(.x, probs = c(0.025, 0.975)))
res_borrow_tbl_cov <- summarize_draws(res_borrow_cov$draws(), ~ quantile(.x, probs = c(0.025, 0.975)))
