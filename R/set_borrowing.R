...
#' historical controls
#' @param baseline_log_hazard_rate_or_odds_prior object of class `Prior`
...
set_borrowing <- function(method,
   ...
   if(method == "BDB" && (
      ...
      is.null(baseline_log_hazard_rate_or_odds_prior)
   )) {
      stop(paste0("When method = 'BDB', ext_flag_col, ",
           "tau_prior, and baseline_log_hazard_rate_or_odds_prior must be ",
           "specified"))
   }

   if(is.null(baseline_log_hazard_rate_or_odds_prior)) {
      stop(paste0("baseline_log_hazard_rate_or_odds_prior must be ",
                  "specified"))
   }
   ...
   if(method == "BDB" && !is(baseline_log_hazard_rate_or_odds_prior, "Prior")) {
      stop("baseline_log_hazard_rate_or_odds_prior prior bust be of class `Prior`")
   }

   .borrowing_class(method = method,
                    ext_flag_col = ext_flag_col,
                    tau_prior = tau_prior,
                    baseline_log_hazard_rate_or_odds_prior =
                       baseline_log_hazard_rate_or_odds_prior)
}
